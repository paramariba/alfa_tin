...
function StatusDot({source}:{source?:string}){const live=source==='finam';const label=live?'Live · Finam':source==='moex'?'MOEX · задержка ~15 мин':'Демо-данные';return <span className={`status ${live?'live':''}`}><i/>{label}</span>}
...
function MarketPage({instruments,select,openPiggy,streakState,onStreakClick,missionTargetId}:{instruments:Instrument[];select:(i:Instrument)=>void;openPiggy:()=>void;streakState?:StreakState;onStreakClick:()=>void;missionTargetId:number|null}){
  ...
  return <motion.main className="page market-page" initial={{opacity:0,y:8}} animate={{opacity:1,y:0}}>
    ...
    <div className="market-summary"><div><span>Рынок сегодня</span><strong>{instruments.filter(i=>i.change_pct>0).length} растут</strong></div><StatusDot source={instruments.find(i=>i.source==='finam')?.source||instruments.find(i=>i.source==='moex')?.source}/></div>
...
function SocialPage({fallbackTitle,streakState,onStreakClick,onOpenUser,onOpenInstrument,version,notify}:{fallbackTitle:string;streakState?:StreakState;onStreakClick:()=>void;onOpenUser:(id:number)=>void;onOpenInstrument:(id:number)=>void;version:number;notify:(text:string)=>void}){
  ...
  return <motion.main className="page social-page" initial={{opacity:0,y:8}} animate={{opacity:1,y:0}}><AppHeader title={data?.title||fallbackTitle} sub="Посты о решениях сообщества" streakState={streakState} onStreakClick={onStreakClick}/>
    <section className="social-intro"><MessageCircle size={25}/><div><strong>Комментарии всего сообщества</strong><p>Здесь видны публикации всех участников. Упомяни инструмент через @тикер — пост прикрепит ссылку, а данные твоих транзакций останутся приватными.</p></div></section>
    ...
    <div className="tabs social-tabs"><button className={scope==='community'?'active':''} onClick={()=>setScope('community')}>Все комментарии</button><button className={scope==='friends'?'active':''} onClick={()=>setScope('friends')}>Друзья · {data?.friends?.length||0}</button></div>
...
function InstrumentSheet({instrument,onClose,refresh,done,onShare}:{instrument:Instrument;onClose:()=>void;refresh:()=>void;done:(s:string)=>void;onShare:(trade:any)=>void}){
  ...
  const [tradeCost,setTradeCost]=useState<{buy:any|null;sell:any|null}>({buy:null,sell:null})
  const [previewBusy,setPreviewBusy]=useState(false)
  useEffect(()=>{api<Instrument>(`/market/instruments/${instrument.id}`).then(setDetails)},[instrument.id])
  const positionQuantity=Number(details.position?.quantity||0)
  useEffect(()=>{
    let cancelled=false
    if(!Number.isFinite(qty)||qty<=0){setTradeCost({buy:null,sell:null});setPreviewBusy(false);return}
    setPreviewBusy(true)
    const timer=window.setTimeout(async()=>{
      const body=JSON.stringify({instrument_id:instrument.id,quantity:qty})
      const buyRequest=api<any>('/trades/preview?side=buy',{method:'POST',body}).catch(()=>null)
      const sellRequest=positionQuantity>=qty?api<any>('/trades/preview?side=sell',{method:'POST',body}).catch(()=>null):Promise.resolve(null)
      const [buy,sell]=await Promise.all([buyRequest,sellRequest])
      if(!cancelled){setTradeCost({buy,sell});setPreviewBusy(false)}
    },180)
    return()=>{cancelled=true;window.clearTimeout(timer)}
  },[instrument.id,qty,positionQuantity])
  const buyTotal=Math.abs(Number(tradeCost.buy?.cash_change??(Number(details.display_price_tkn||0)*Math.max(0,qty))))
  const sellTotal=tradeCost.sell?Number(tradeCost.sell.cash_change):null
  const insufficientCash=tradeCost.buy?.enough_cash===false
  const insufficientPosition=!details.position||qty>positionQuantity
    ...
    const connect=()=>{
      ...
      socket.onmessage=(event)=>{
        try{
          ...
          const source=message.data.some((item:Instrument)=>item.source==='finam')?'finam':message.data.some((item:Instrument)=>item.source==='moex')?'moex':'demo'
          const live=source==='finam'
          setInstruments(current=>current.map(item=>updates.has(item.id)?{...item,...updates.get(item.id)!}:item))
          setDashboard((current:any)=>({...current,market_status:{label:live?'Live · Finam':source==='moex'?'MOEX · задержка ~15 мин':'Демо-данные',timestamp:message.timestamp,live,source}}))
